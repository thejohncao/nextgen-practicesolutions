...
function AgentCard({activeAgent}: {activeAgent: string}) {
  ...
  // Build stack of other agents behind active card
      ...
      <div style={{position:"relative",width:540,height:480,flexShrink:0}}>
        ...
        <div style={{position:"absolute",inset:-60,pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
          <div style={{
            ...
            animation:"bg-orb-drift-1 8s ease-in-out infinite",
            opacity:0.12,willChange:"transform,opacity",
          }}/>
          <div style={{
            position:"absolute",bottom:"15%",right:"10%",width:160,height:160,
            borderRadius:"50%",background:col,filter:"blur(70px)",
            animation:"bg-orb-drift-2 10s ease-in-out infinite",
            opacity:0.10,willChange:"transform,opacity",
          }}/>
          <div style={{
            position:"absolute",top:"55%",left:"55%",width:120,height:120,
            borderRadius:"50%",background:col,filter:"blur(60px)",
            animation:"bg-orb-drift-3 7s ease-in-out infinite",
            opacity:0.08,willChange:"transform,opacity",
          }}/>
          {/* Sweeping line */}
          <div style={{
            position:"absolute",top:"50%",left:"-20%",width:"140%",height:1,
            background:`linear-gradient(90deg, transparent 0%, ${col}44 30%, ${col}22 70%, transparent 100%)`,
            animation:"sweep-line 6s ease-in-out infinite",
            opacity:0.5,willChange:"transform",
          }}/>
        </div>

        {/* Stacked background cards (z-index layered) */}
        {otherAgents.slice(0,3).map((bg, i) => {
          const bgCol = AGENT_COLOR[bg.id];
          const stackIndex = i;
          const offsetX = 14 + stackIndex * 18;
          const offsetY = 28 + stackIndex * 36;
          const scale = 0.97 - stackIndex * 0.02;
          const rotate = (stackIndex + 1) * 1.2;
          const cardOpacity = 0.65 - stackIndex * 0.12;
          return (
            <div key={bg.id} style={{
              position:"absolute",
              top: offsetY,
        ...
        <div style={{width:440,height:340,position:"relative",perspective:2000,cursor:"pointer",zIndex:10,
