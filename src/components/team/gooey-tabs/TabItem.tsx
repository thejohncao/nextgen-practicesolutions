...
const TabItem = ({ agent, index, activeTab, onClick }: TabItemProps) => {
  ...
  const getTabBgClass = (color: string, isActive: boolean): string => {
    if (!isActive) return "bg-white/5 hover:bg-white/10";
  ...
  const getAgentIcon = (agentName: string) => {
    switch(agentName) {
      case 'Giselle': return <Fuel className={`w-4 h-4 ${isActive ? 'text-green-400' : 'text-white/50'}`} />;
      case 'Miles': return <Calendar className={`w-4 h-4 ${isActive ? 'text-blue-400' : 'text-white/50'}`} />;
      case 'Devon': return <Handshake className={`w-4 h-4 ${isActive ? 'text-purple-400' : 'text-white/50'}`} />;
      case 'Alma': return <GraduationCap className={`w-4 h-4 ${isActive ? 'text-amber-400' : 'text-white/50'}`} />;
      default: return null;
    }
  };
  ...
  return (
    <motion.button 
      onClick={onClick}
      className={cn(
        "relative z-10 flex items-center px-6 py-3 rounded-tl-lg rounded-tr-lg",
        ...
        isActive ? "text-white font-medium shadow-lg" : "text-white/70"
      )}
      initial={false}
      animate={{ 
        y: isActive ? -4 : 0,
